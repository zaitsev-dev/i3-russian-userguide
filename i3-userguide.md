...
Рекомендуется использовать коментарии в конфигурационном файле, чтобы правильно задокументировать ваши настройки для последующего использования. Коментарии начинаются с символа **#** и могут быть использованы только в начале строки:

`# Это строка с коментарием`
...
Действующие критерии (условия) и команды см. в разделе command_criteria (TODO: добавить ссылку). Выполняться могут только команды, а не директивы конфигурации, см. раздел list_of_commands (TODO: добавить ссылку).
...
Смотрите [command_chaining]()(TODO: добавить ссылку) для получения подробной информации об особом значении символов `;` (точка с запятой) и `,` (запятая). Они связывают вместе команды в i3, поэтому необходимо использовать кавычки (как показано в [exec_quoting]()(TODO: добавить ссылку)), если они используются в вашей команде.
...
Про флаг `--no-startup-id` рассказывается в [exec]()(TODO: добавить ссылку).
